...
def find_highlights(transcript: dict, max_clips: int) -> list[ClipSegment]:
    ...
    prompt = f"""You select viral short-form moments from a timestamped transcript.
Return exactly {max_clips} or fewer clips. Each must be 15-45 seconds, must stay inside the transcript, and should have a strong hook, punchline, emotion, useful insight, conflict, surprise, or high-energy moment.
Return ONLY JSON in this shape: {{\"clips\":[{{\"start\":12.3,\"end\":35.2,\"title\":\"short title\",\"reason\":\"why it works\"}}]}}
...
def build_ass(transcript: dict, start: float, end: float, path: Path) -> None:
    ...
    for seg in transcript["segments"]:
        if seg["end"] < start or seg["start"] > end:
            continue
        a = max(0.0, seg["start"] - start)
        b = min(end - start, seg["end"] - start)
...
def render_clip(source: Path, transcript: dict, clip: ClipSegment, out: Path, watermark: Optional[Path]) -> None:
    ...
    build_ass(transcript, clip.start, clip.end, ass)
    duration = clip.end - clip.start
    ass_filter = f"ass={ass.as_posix()}"
    if watermark and watermark.exists():
        cmd = ["ffmpeg", "-y", "-ss", str(clip.start), "-t", str(duration), "-i", str(source), "-i", str(watermark),
               "-filter_complex", f"[0:v]crop=ih*9/16:ih,scale=1080:1920[c];[1:v]scale=220:-1[wm];[c][wm]overlay=W-w-30:30,{ass_filter}[v]",
               "-map", "[v]", "-map", "0:a?", "-c:v", "libx264", "-preset", "fast", "-crf", "20", "-c:a", "aac", "-b:a", "160k", str(out)]
    else:
        cmd = ["ffmpeg", "-y", "-ss", str(clip.start), "-t", str(duration), "-i", str(source),
               "-vf", f"crop=ih*9/16:ih,scale=1080:1920,{ass_filter}", "-map", "0:v", "-map", "0:a?",
               "-c:v", "libx264", "-preset", "fast", "-crf", "20", "-c:a", "aac", "-b:a", "160k", str(out)]
    run_cmd(cmd)
...
def pipeline(job_id: str, video: Path, watermark: Optional[Path], max_clips: int) -> None:
    try:
        ...
        JOBS[job_id]["status"] = "finding_highlights"
        clips = find_highlights(transcript, max_clips)
        JOBS[job_id]["status"] = "rendering"
        results = []
        for i, clip in enumerate(clips):
            out = video.parent / f"clip_{i}.mp4"
            render_clip(video, transcript, clip, out, watermark)
...
@app.get("/")
def root():
    return {"status": "ok", "service": "AutoClipper V2"}
...
async def process_video(background_tasks: BackgroundTasks, file: UploadFile = File(...), watermark: Optional[UploadFile] = File(None), max_clips: int = 5):
    ...
    if watermark:
        ...
        with watermark_path.open("wb") as out:
            shutil.copyfileobj(watermark.file, out)
    JOBS[job_id] = {"status": "queued", "clips": []}
    background_tasks.add_task(pipeline, job_id, video, watermark_path, max_clips)
