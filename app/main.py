...
app=FastAPI(title='AutoClipper V4',version='4.0.0'); app.add_middleware(CORSMiddleware,allow_origins=['*'],allow_methods=['*'],allow_headers=['*']); JOBS={}
...
def render_clip(source,transcript,clip,out,watermark):
    ...
    if watermark and watermark.exists():
        ...
        cmd=['ffmpeg','-y','-threads','0','-ss',str(clip.start),'-t',str(dur),'-i',str(source),'-vf',f'crop=ih*9/16:ih,scale=1080:1920,{af}','-map','0:v','-map','0:a?']+codec+['-c:a','aac','-b:a','128k','-movflags','+faststart','-loglevel','error',str(out)]
...
def pipeline(job_id,video,watermark,max_clips,instruction):
    try:
        ...
        args=[(job_id,video,transcript,c,i,watermark) for i,c in enumerate(clips)]; results=[None]*len(args)
        with ThreadPoolExecutor(max_workers=min(RENDER_WORKERS,len(args))) as pool:
            ...
            for f in as_completed(futures):
                i=futures[f]; results[i]=f.result(); completed+=1; JOBS[job_id]['completed_clips']=completed; JOBS[job_id]['progress']=40+int(completed/len(args)*50)
...
@app.get('/health')
def health(): return {'status':'healthy','version':'4.0.0','openai_configured':bool(OPENAI_API_KEY),'text_model':TEXT_MODEL,'transcribe_model':TRANSCRIBE_MODEL,'render_workers':RENDER_WORKERS,'encoder':VIDEO_ENCODER}
@app.post('/process')
async def process_video(background_tasks:BackgroundTasks,file:UploadFile=File(...),watermark:Optional[UploadFile]=File(None),max_clips:int=5,instruction:str=''):
    ...
    JOBS[job_id]={'status':'queued','progress':0,'clips':[],'instruction':instruction}; background_tasks.add_task(pipeline,job_id,video,wm,max_clips,instruction); return {'job_id':job_id,'status':'queued','version':'4.0.0'}
