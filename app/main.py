...
ENCODER = os.getenv('AUTOCLIPPER_ENCODER', 'auto')
API_KEY = os.getenv('AUTOCLIPPER_API_KEY', '')
JOB_TTL_HOURS = int(os.getenv('JOB_TTL_HOURS', '24'))
UPLOAD_TTL_HOURS = int(os.getenv('UPLOAD_TTL_HOURS', '6'))

client = OpenAI(api_key=OPENAI_API_KEY) if OPENAI_API_KEY else None
app = FastAPI(title='AutoClipper V4', version='4.1.0')
...
def run_cmd(cmd):
    ...
    if r.returncode:
        raise RuntimeError(r.stderr[-4000:] or 'ffmpeg failed')
    return r.stdout
...
def choose_encoder():
    ...
    try:
        ...
        if 'h264_nvenc' in enc and shutil.which('nvidia-smi'):
            return 'h264_nvenc'
    except Exception:
        pass
    return 'libx264'
...
def cleanup_old_jobs():
    ...
    for d in WORK_DIR.iterdir():
        try:
            if d.is_dir() and d.name != '_uploads' and d.stat().st_mtime < cutoff:
...
def transcribe(video_path, job_dir):
    ...
    run_cmd(['ffmpeg', '-y', '-threads', '0', '-i', str(video_path), '-vn', '-ac', '1', '-ar', '16000', '-b:a', '24k', str(audio)])
    if audio.stat().st_size > 24 * 1024 * 1024:
        raise RuntimeError('Audio is too large for transcription. Use a shorter video.')
...
TRANSCRIPT:
{lines}'''
    r = client.chat.completions.create(model=TEXT_MODEL, messages=[{'role': 'user', 'content': prompt}], response_format={'type': 'json_object'})
    ...
    for x in data.get('clips', [])[:max_clips]:
        try:
            ...
            continue
        if not (end > start and 15 <= end - start <= 60):
            continue
    ...
    selected = []
    for c in clips:
        if all(c.end <= p.start + 2 or c.start >= p.end - 2 for p in selected):
            selected.append(c)
    if not selected:
        raise RuntimeError('AI did not return valid distinct clip segments')
    return selected
...
'''
    ...
    for s in transcript['segments']:
        ...
        events.append(f'Dialogue: 0,{ass_time(a)},{ass_time(b)},Default,{t}')
...
def render_clip(source, transcript, clip, out, watermark):
    ...
    if watermark and watermark.exists():
        fc = f'[0:v]{base}[c];[1:v]scale=220:-1[wm];[c][wm]overlay=W-w-30:30,{af}[v]'
        cmd = ['ffmpeg', '-y', '-threads', '0', '-ss', str(clip.start), '-t', str(dur), '-i', str(source), '-i', str(watermark), '-filter_complex', fc, '-map', '[v]', '-map', '0:a?'] + codec + ['-c:a', 'aac', '-b:a', '128k', '-movflags', '+faststart', '-loglevel', 'error', str(out)]
    else:
        cmd = ['ffmpeg', '-y', '-threads', '0', '-ss', str(clip.start), '-t', str(dur), '-i', str(source), '-vf', f'{base},{af}', '-map', '0:v', '-map', '0:a?'] + codec + ['-c:a', 'aac', '-b:a', '128k', '-movflags', '+faststart', '-loglevel', 'error', str(out)]
...
def render_one(args):
    job_id, video, transcript, clip, i, wm = args
    out = video.parent / f'clip_{i}.mp4'; render_clip(video, transcript, clip, out, wm)
    return {'index': i, **clip.model_dump(), 'download_url': f'/download/{job_id}/{i}'}
...
def pipeline(job_id, video, watermark, max_clips, instruction):
    try:
        ...
        JOBS[job_id]['status'] = 'scoring_highlights'; JOBS[job_id]['progress'] = 35
        clips = find_highlights(transcript, max_clips, instruction)
        JOBS[job_id].update(status='rendering', progress=40, total_clips=len(clips), completed_clips=0, render_workers=min(RENDER_WORKERS, len(clips)), encoder=VIDEO_ENCODER)
        ...
        JOBS[job_id]['status'] = 'creating_social_package'; JOBS[job_id]['progress'] = 95
        JOBS[job_id].update(status='done', progress=100, clips=results, social=social_package(clips, instruction))
...
def root():
    ...
    return HTMLResponse(ui.read_text(encoding='utf-8')) if ui.exists() else HTMLResponse('<h1>AutoClipper V4</h1><p>Frontend not found.</p>', status_code=500)
...
@app.get('/health')
def health():
    return {'status': 'healthy', 'version': '4.1.0', 'openai_configured': bool(OPENAI_API_KEY), 'text_model': TEXT_MODEL, 'transcribe_model': TRANSCRIBE_MODEL, 'render_workers': RENDER_WORKERS, 'encoder': VIDEO_ENCODER, 'chunk_size': CHUNK_SIZE, 'auth_required': bool(API_KEY)}


@app.post('/upload/init')
def upload_init(filename: str = '', size: int = 0):
    cleanup_old_uploads()
    if not filename:
        raise HTTPException(400, 'filename is required')
    if size <= 0:
        raise HTTPException(400, 'file size is required')
    if size > MAX_UPLOAD_MB * 1024 * 1024:
        raise HTTPException(413, f'Video exceeds {MAX_UPLOAD_MB} MB')
    upload_id = uuid.uuid4().hex
    directory = UPLOAD_DIR / upload_id
    directory.mkdir(parents=True, exist_ok=False)
    (directory / 'meta.json').write_text(json.dumps({'filename': Path(filename).name, 'size': size, 'chunk_size': CHUNK_SIZE}), encoding='utf-8')
    return {'upload_id': upload_id, 'chunk_size': CHUNK_SIZE, 'status': 'initialized'}
...
async def upload_chunk(upload_id: str, chunk_index: int, chunk: UploadFile = File(...)):
    ...
    try:
        ...
        max_chunk = int(meta['chunk_size'])
    except Exception:
        raise HTTPException(500, 'Upload session metadata is corrupt')
    target = directory / f'{chunk_index:08d}.part'
    temp = directory / f'.{chunk_index:08d}.uploading'
    if target.exists():
        return {'upload_id': upload_id, 'chunk_index': chunk_index, 'received': target.stat().st_size, 'status': 'already_uploaded'}
    received = 0
    try:
        with temp.open('wb') as out:
            while True:
                ...
                if received > max_chunk:
                    raise HTTPException(413, f'Chunk exceeds {max_chunk} bytes')
                out.write(data)
        ...
        temp.unlink(missing_ok=True)
        raise HTTPException(400, f'Failed to save chunk: {e}')
...
def upload_complete(upload_id: str, body: UploadComplete, background_tasks: BackgroundTasks):
    ...
    if not directory.is_dir() or not meta_path.exists():
        raise HTTPException(404, 'Upload session not found')
    if body.chunks < 1 or body.chunks > (MAX_UPLOAD_MB * 1024 * 1024 // CHUNK_SIZE) + 1:
        raise HTTPException(400, 'Invalid chunk count')
    meta = json.loads(meta_path.read_text(encoding='utf-8'))
    expected_size = int(meta['size'])
    parts = [directory / f'{i:08d}.part' for i in range(body.chunks)]
    missing = [i for i, p in enumerate(parts) if not p.exists()]
    if missing:
        raise HTTPException(409, f'Missing chunks: {missing[:10]}')
    total = sum(p.stat().st_size for p in parts)
    if total != expected_size:
        raise HTTPException(409, f'Upload size mismatch: received {total}, expected {expected_size}')
    ...
    try:
        ...
        if video.stat().st_size != expected_size:
            raise RuntimeError('Assembled video size mismatch')
    except Exception as e:
        shutil.rmtree(job_dir, ignore_errors=True)
        raise HTTPException(400, f'Failed to assemble upload: {e}')
    shutil.rmtree(directory, ignore_errors=True)
    max_clips = max(1, min(int(body.max_clips), 10))
    JOBS[job_id] = {'status': 'queued', 'progress': 0, 'clips': [], 'instruction': body.instruction}
    background_tasks.add_task(pipeline, job_id, video, None, max_clips, body.instruction)
    return {'job_id': job_id, 'status': 'queued', 'version': '4.1.0'}
...
async def process_video(background_tasks: BackgroundTasks, file: UploadFile = File(...), watermark: Optional[UploadFile] = File(None), max_clips: int = 5, instruction: str = '', x_api_key: Optional[str] = Header(None)):
    ...
    max_bytes = MAX_UPLOAD_MB * 1024 * 1024; size = 0
    try:
        with video.open('wb') as out:
            while True:
        ...
        shutil.rmtree(job_dir, ignore_errors=True); raise HTTPException(400, 'Failed to save uploaded video')
    wm = None
    if watermark:
    ...
    background_tasks.add_task(pipeline, job_id, video, wm, max_clips, instruction)
    return {'job_id': job_id, 'status': 'queued', 'version': '4.1.0'}
