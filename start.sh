...
if [ -f "$BGUTIL_DIR/build/main.js" ]; then
  cd "$BGUTIL_DIR"
  node build/main.js --host 127.0.0.1 --port 4416 >"$BGUTIL_LOG" 2>&1 &
  BGUTIL_PID=$!
else
...
for i in $(seq 1 30); do
  if curl -fsS http://127.0.0.1:4416/ >/dev/null 2>&1 || curl -fsS http://127.0.0.1:4416/health >/dev/null 2>&1; then
    READY=1
    break
  fi
